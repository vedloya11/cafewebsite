...
function placeOrder() {
  ...
  orders.unshift({
    id,
    tableId: document.getElementById('tableLabel').textContent,
    items: items.map(i => ({...i})),
    note,
    status: currentAdmin ? 'served' : 'pending',
    time: new Date(),
...
function adminTab(el, tab) {
  document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
  el.classList.add('active');
  ['orders','tables','billing','summary','menu-manage'].forEach(t => {
    const container = document.getElementById(`tab-${t}`);
    if (container) container.style.display = t===tab ? 'block' : 'none';
  });
  const titles = {orders:'Live Orders', tables:'Table Occupancy', billing:'Billing', summary:"Today's Summary", 'menu-manage':'Manage Menu'};
  document.getElementById('adminTabTitle').textContent = titles[tab];
  activeAdminTab = tab;
  renderAdminTab();
}
...
function renderTables() {
  ...
  document.getElementById('tablesGrid').innerHTML = TABLES.map(t => {
    const info = occupied[t];
    const isBilled = info && info.orders.every(o => o.status === 'served');
    let cls = 'free', statusText = 'Available', amountText = 'Open';
    if (info) {
      cls = 'occupied';
      statusText = `${info.orders.length} order${info.orders.length>1?'s':''}`;
...
// QR code simulation: detect table from URL hash
const hash = window.location.hash;
if (hash && hash.startsWith('#table=')) {
  document.getElementById('tableLabel').textContent = hash.replace('#table=','').toUpperCase();
}
